...
static long cbctrl_ioctl( struct file *file, unsigned int cmd, unsigned long arg )
{
   ...
   void __iomem *offset = lp->base_addr + data.ioOffset;
   dev_info( lp->dev, "Base: %08X, ioOffset: %08X, Offset %08X\n", lp->base_addr, data.ioOffset, offset );
   ...
   switch ( cmd )
   {
      case READ_REG_IOCTL:
         ...
         data.ioData = cbctrl_readreg( offset );
      ...
      case WRITE_REG_IOCTL:
         ...
         copy_from_user( &data, ( cbctrl_t * )arg, sizeof( cbctrl_t ) );
         dev_info( lp->dev, "Data: %08X\n", data.ioData);
         cbctrl_writereg( data.ioData, offset );
                            ...
                            dev_t         *dev_node )
{
   ...
   rc = alloc_chrdev_cbctrln( dev_node, 0, 1, "cbctrl" );
...
init_error1:
   unregister_chrdev_cbctrln( *dev_node, 1 );
